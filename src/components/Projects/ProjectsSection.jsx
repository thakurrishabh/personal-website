...
import ClusterNode from '../Work/ClusterNode';
import NetworkTrunk from '../Shared/NetworkTrunk';
import CloudRouter from '../Shared/CloudRouter';

const ProjectsSection = () => {
    return (
        <section className="relative w-full min-h-screen pt-0 pb-32 bg-[#0B1120] overflow-hidden" id="projects">
            {/* Background Texture - Grid */}
            <div className="absolute inset-0 opacity-[0.03] pointer-events-none active-grid-pattern"></div>

            {/* 1. Main VPC Connector Channel (The Big Tube) - Perfectly Aligned with Hero & Work */}
            ...
            {/* 3. The Projects Subnet (Inner Box) */}
            <div className="relative z-10 container mx-auto px-4 pt-32">
                {/* Router - Top Connection Point */}
                <div className="flex justify-center -mb-4 relative z-20">
                    <CloudRouter label="PROJECTS_INGRESS_GW" direction="up" />
                </div>

                <div className="relative w-full max-w-[1400px] mx-auto border border-purple-500/20 bg-slate-900/20 rounded-3xl p-6 md:p-12 shadow-2xl backdrop-blur-sm">

                    {/* Subnet Header */}
                    <div className="absolute top-6 left-6 md:left-8 px-3 py-1 bg-[#0B1120] border border-purple-500/30 rounded-full flex items-center gap-2 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
                        <div className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse"></div>
                        <span className="text-[10px] font-mono text-purple-400 font-bold tracking-widest uppercase">
                            SUBNET: projects-cluster (k8s)
                        </span>
                    </div>

                    <div className="text-center mb-16 mt-16">
                        <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-3">
                            Personal <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Projects</span>
                        </h2>
                        <div className="inline-block px-4 py-1.5 rounded-md bg-slate-950 border border-slate-800 text-slate-400 font-mono text-xs">
                            <span className="text-purple-500">$</span> kubectl get nodes --namespace=projects
                        </div>
                    </div>
                ...
                {/* Router - Bottom Connection */}
                <div className="absolute -bottom-[20px] left-1/2 -translate-x-1/2 z-20">
                    <CloudRouter label="PROJECTS_GATEWAY" direction="down" />
                </div>
            </div>

            {/* Bottom Trunk Connecting Projects to next section */}
